...
namespace {
  ...
  parseChannelDescriptor(const std::string_view paID, const std::size_t paStart, ComChannelDescriptor &paResult) {
    ...
    paResult = {
        .mChannel = CStringDictionary::getId(paID.data() + paStart, openingBracket - paStart),
...
std::string_view forte::com::getComResultString(const ComResult paResult) {
  switch (paResult) {
...
std::vector<ComChannelDescriptor> forte::com::parseComId(const std::string_view paId) {
