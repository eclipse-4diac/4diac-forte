 ...
 *    Peter Gsellmann - changed to CEventChainExecutionThreadAbstract as
...
class CEventSourceFB : public CFunctionBlock {
  private:
     ...
     */
    CEventChainExecutionThreadAbstract *m_poEventChainExecutor;
    SEventEntry mEventSourceEventEntry; //! the event entry to start the event chain

  public:
    CEventSourceFB(CResource *pa_poSrcRes, const SFBInterfaceSpec *pa_pstInterfaceSpec, const CStringDictionary::TStringId pa_nInstanceNameId,
        TForteByte *pa_acFBConnData, TForteByte *pa_acFBVarsData) :
        CFunctionBlock(pa_poSrcRes, pa_pstInterfaceSpec, pa_nInstanceNameId, pa_acFBConnData, pa_acFBVarsData), m_poEventChainExecutor(0),
            mEventSourceEventEntry(this, cg_nExternalEventID) {
    }
    virtual ~CEventSourceFB() {
    }
    ;
    void setEventChainExecutor(CEventChainExecutionThreadAbstract *pa_poEventChainExecutor) {
      m_poEventChainExecutor = pa_poEventChainExecutor;
    }
    ;
    CEventChainExecutionThreadAbstract* getEventChainExecutor(void) {
      return m_poEventChainExecutor;
    }
    ;

    SEventEntry* getEventSourceEventEntry(void) {
