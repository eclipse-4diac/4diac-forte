 ...
 *    Peter Gsellmann, Martin Melik Merkumians - added postFBcreateAction method
 *******************************************************************************/
...
CFunctionBlock* CFBContainer::getFB(CStringDictionary::TStringId paFBName) {
  ...
  if(CStringDictionary::scm_nInvalidStringId != paFBName) {
    for(TFunctionBlockList::Iterator it = mFunctionBlocks.begin(); it != mFunctionBlocks.end(); ++it) {
...
CFBContainer* CFBContainer::getFBContainer(CStringDictionary::TStringId paContainerName) {
  ...
  if(CStringDictionary::scm_nInvalidStringId != paContainerName) {
    for(TFBContainerList::Iterator it = mSubContainers.begin(); it != mSubContainers.end(); ++it) {
...
void forte::core::CFBContainer::postFBcreateAction(CFunctionBlock &paNewFB) {
}

EMGMResponse CFBContainer::changeContainedFBsExecutionState(EMGMCommandType paCommand) {
  EMGMResponse retVal = e_RDY;

  for(TFBContainerList::Iterator it(mSubContainers.begin()); ((it != mSubContainers.end()) && (e_RDY == retVal)); ++it) {
    retVal = (*it)->changeContainedFBsExecutionState(paCommand);
  }

  if(e_RDY == retVal) {
    for(TFunctionBlockList::Iterator itRunner(mFunctionBlocks.begin()); ((itRunner != mFunctionBlocks.end()) && (e_RDY == retVal)); ++itRunner) {
