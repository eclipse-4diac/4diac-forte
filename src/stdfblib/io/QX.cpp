...
const TDataIOID FORTE_QX::scm_anEIWith[] = {0, 1, scmWithListDelimiter, 0, 2, scmWithListDelimiter};
...
const TDataIOID FORTE_QX::scm_anEOWith[] = {0, 1, scmWithListDelimiter, 0, 1, scmWithListDelimiter};
...
FORTE_QX::FORTE_QX(const CStringDictionary::TStringId pa_nInstanceNameId, CResource *pa_poSrcRes) :
...
CIEC_ANY *FORTE_QX::getDIO(size_t) {
  return nullptr;
}
...
CInOutDataConnection **FORTE_QX::getDIOInConUnchecked(TPortId) {
  return nullptr;
}

CInOutDataConnection *FORTE_QX::getDIOOutConUnchecked(TPortId) {
  return nullptr;
}
