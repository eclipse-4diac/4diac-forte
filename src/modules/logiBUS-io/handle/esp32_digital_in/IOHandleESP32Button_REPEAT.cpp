...
IOHandleESP32Button_REPEAT::IOHandleESP32Button_REPEAT(logiBUSIODeviceController *paDeviceCtrl, forte::core::io::IOMapper::Direction paDirection, CIEC_ANY::EDataTypeID paDataType, logiBUSIOEnums::DigitalInPinNumber paPin)
...
static const button_event_t EVENT = BUTTON_PRESS_REPEAT;

void IOHandleESP32Button_REPEAT::register_GPIO_BUTTON(logiBUSIOEnums::DigitalInPinNumber paPin, int32_t gpio_num) {
...
void IOHandleESP32Button_REPEAT::button_cb(void *button_handle, void *usr_data) {
  if(usr_data) {
    IOHandleESP32Button_REPEAT *p = static_cast<IOHandleESP32Button_REPEAT *>(usr_data);
    p->button_cb(button_handle);
  }
}

void IOHandleESP32Button_REPEAT::button_cb(void *button_handle) {
  uint8_t repeat = iot_button_get_repeat(static_cast<button_handle_t>(button_handle));
...
void IOHandleESP32Button_REPEAT::get(CIEC_ANY &paState) {
  static_cast<CIEC_BYTE &>(paState) = CIEC_BYTE(this->repeat);
}

IOHandleESP32Button_REPEAT::~IOHandleESP32Button_REPEAT() {
...
void IOHandleESP32Button_REPEAT::set(const CIEC_ANY &paState) {
}
