...
class FORTE_RT_E_MERGE final : public CRTEventSingle {
  ...
  private:
    ...
    bool checkActivation(TEventID) override {
      return true;
    } // we activate on every input event
  ...
  public:
    FORTE_RT_E_MERGE(const forte::core::InstanceNameId paInstanceNameId, forte::core::CFBContainer &paContainer);
    ...
    void evt_EI2(const CIEC_BOOL &pa_QI,
                 const CIEC_TIME &pa_Tmin,
                 const CIEC_TIME &pa_Deadline,
                 const CIEC_TIME &pa_WCET,
                 CIEC_BOOL &pa_QO) {
