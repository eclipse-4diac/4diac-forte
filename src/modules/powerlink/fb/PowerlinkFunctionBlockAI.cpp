...
void PowerlinkFunctionBlockAI::cnSynchCallback() {
  ...
  for (const auto &mappingValue : eplMapping.mCurrentValues) {
    short ioVal = 0x0000;
    char lowByte =
        (procImageOut[mappingValue->mPiOffset] & (0xFF << mappingValue->mBitOffset)) >> mappingValue->mBitOffset;
    char highByte =
        (procImageOut[mappingValue->mPiOffset + 1] & (0xFF << mappingValue->mBitOffset)) >> mappingValue->mBitOffset;
...
void PowerlinkFunctionBlockAI::executePowerlinkEvent(const TEventID paEIID,
  ...
  if (paEIID == scmEventINITID) {
    ...
    if (var_QI == true && initOk) {
      ...
      for (TPortId i = 3; i < getFBInterfaceSpec().getNumDOs() && it != itEnd; i++, ++it) {
        short ioVal = 0x0000;
