...
bool GEN_STRUCT_DEMUX::createInterfaceSpec(const char *paConfigString, SFBInterfaceSpec &paInterfaceSpec) {
  ...
  if(0 != data){
    if(data->getDataTypeID() == CIEC_ANY::e_STRUCT){
      ...
      TDataIOID* eoWith = new TDataIOID[structInstance->getStructSize() + 1];
      CStringDictionary::TStringId* doDataTypeNames = new CStringDictionary::TStringId[structInstance->getStructSize()];
      CStringDictionary::TStringId* doNames = new CStringDictionary::TStringId[structInstance->getStructSize()];
      ...
      paInterfaceSpec.m_nNumDOs = static_cast<TForteUInt8>(structInstance->getStructSize());
      ...
      DEVLOG_DEBUG("GEN_STRUCT_DEMUX: data type is not a struct: %s\n", CStringDictionary::getInstance().get(structTypeNameId));
    }
    delete data;
  } else {
    DEVLOG_DEBUG("GEN_STRUCT_DEMUX: Couldn't create struct of type: %s\n", CStringDictionary::getInstance().get(structTypeNameId));
  }
  return retval;
}
