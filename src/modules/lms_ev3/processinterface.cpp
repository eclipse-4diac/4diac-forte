...
CLMSEV3ProcessInterface::CLMSEV3ProcessInterface(forte::core::CFBContainer &paContainer, const SFBInterfaceSpec& paInterfaceSpec, const CStringDictionary::TStringId paInstanceNameId) :
  ...
  if (mFile.is_open()){
    ...
    switch (mnTypeOfIO){
      case MOTOR_POSITION:{
        ...
        if (std::numeric_limits<CIEC_INT::TValueType>::min() <= finalVal && std::numeric_limits<CIEC_INT::TValueType>::max() >= finalVal){
          mFile << finalVal;
          writeAttempted = true;
        }else{
          STATUS() = scmCouldNotWrite;
