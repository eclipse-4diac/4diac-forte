...
const SFBInterfaceSpec FB_CTD_DINT::scm_stFBInterfaceSpec = { 1, scm_anEventInputNames, scm_anEIWith, scm_anEIWithIndexes, 1, scm_anEventOutputNames,
...
void FB_CTD_DINT::executeEvent(int pa_nEIID) {
  if(pa_nEIID == scm_nEventREQID) {
    if(true == LD()) {
      CV() = PV();
    } else if(func_AND(CD(), func_GT(CV(), CIEC_DINT(CIEC_DINT::scm_nMinVal)))) {
      CV() = func_SUB(CV(), CIEC_DINT(1));
    }
    Q() = func_LE(CV(), CIEC_DINT(0));
