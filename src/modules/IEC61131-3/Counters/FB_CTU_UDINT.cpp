...
const SFBInterfaceSpec FB_CTU_UDINT::scm_stFBInterfaceSpec = { 1, scm_anEventInputNames, scm_anEIWith, scm_anEIWithIndexes, 1, scm_anEventOutputNames,
...
void FB_CTU_UDINT::executeEvent(int pa_nEIID) {
  if(pa_nEIID == scm_nEventREQID) {
    if(true == R()) {
      CV() = CIEC_UDINT(0);
    } else if(func_AND(CU(), func_LT(CV(), CIEC_UDINT(CIEC_UDINT::scm_nMaxVal)))) {
      CV() = func_ADD(CV(), CIEC_UDINT(1));
    }
    Q() = func_GE(CV(), PV());
