...
const TDataIOID FORTE_FB_CTU_LINT::scm_anEIWith[] = {0, 1, 255};
...
const TDataIOID FORTE_FB_CTU_LINT::scm_anEOWith[] = {0, 1, 255};
...
void FORTE_FB_CTU_LINT::alg_REQ(void) {
  ...
  else if (func_AND<CIEC_BOOL>(var_CU, func_LT(var_CV, CIEC_LINT(9223372036854775807)))) {
...
void FORTE_FB_CTU_LINT::executeEvent(int pa_nEIID){
  switch(pa_nEIID) {
...
void FORTE_FB_CTU_LINT::readInputData(size_t pa_nEIID) {
  switch(pa_nEIID) {
    case scm_nEventREQID: {
...
void FORTE_FB_CTU_LINT::writeOutputData(size_t pa_nEIID) {
  switch(pa_nEIID) {
