 ...
 * periodically action on it This class contains the common code of them and uses a strategy pattern in run(), where the
 * action ot be executed periodically on the clients is done in the pure virtual handleClients(). The iteration can
 * sleep forever or sleep for some time and then iterate again. In both cases, the thread can be awaken.
 */
class COPC_UA_Client_IterationList : public CThread {
  public:
    ...
    /**
     * Adds a new client to the list of clients where handleClients() is executed. The new clients are added
     * asyncrhonously so the iteration doesn't block the thread calling this. The client is usually no longer used by
     * the caller
    ...
    /**
     * Removes a client from the iteration list syncrhonously. Usually, the client is then accessed after removed is
     * then deleted, so the caller has to wait that the client is free before accessing it
    ...
    /**
     * Get the added clients to the iteration list, and execute handleClients(). Depending on the result, it will wait
     * forever or just for some time (the cyclic period)
     */
    void run() override;
  ...
  private:
     ...
     */
    static const TForteUInt64 scmNanosecondsToSleep =
        static_cast<TForteUInt64>(FORTE_COM_OPC_UA_CLIENT_PUB_INTERVAL * 1000000); // ms to nanoseconds
};
