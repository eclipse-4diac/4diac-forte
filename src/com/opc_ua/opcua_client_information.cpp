  ...
  if (CActionInfo::eCallMethod == paActionInfo.getAction()) {
    ...
    for (auto itNodePair = paActionInfo.getNodePairInfo().begin(); itNodePair != paActionInfo.getNodePairInfo().end();
      ...
      if (!somethingFailed &&
        ...
        UA_NodeId *nodeId;
        UA_StatusCode retVal = COPC_UA_Helper::getRemoteNodeForPath(*mClient, itNodePair->getBrowsePath().c_str(),
                                                                    nullptr, &nodeId); // we don't care about the parent
  ...
  if (UA_STATUSCODE_GOOD != retVal) {
    ...
    if (itNodePair->getNodeId() != nullptr) {
      if (!UA_NodeId_equal(itNodePair->getNodeId(),
        ...
        DEVLOG_ERROR("[OPC UA CLIENT]: The method call from FB %s failed the found nodeId of the method doesn't match "
                     "the provided one\n",
                     paActionInfo.getLayer().getCommFB()->getInstanceName());
        somethingFailed = true;
      }
...
bool CUA_ClientInformation::addMonitoringItem(UA_MonitoringItemInfo &paMonitoringInfo, const UA_NodeId &paNodeId) {
  ...
  UA_MonitoredItemCreateResult monResponse =
      UA_Client_MonitoredItems_createDataChange(mClient, mSubscriptionInfo.mSubscriptionId, UA_TIMESTAMPSTORETURN_BOTH,
                                                monRequest, static_cast<void *>(&paMonitoringInfo.mVariableInfo),
...
void CUA_ClientInformation::resetSubscription(bool paDeleteSubscription) {
  ...
  if (paDeleteSubscription) {
    ...
    if (UA_STATUSCODE_GOOD != retval) {
      DEVLOG_ERROR(
          "[OPC UA CLIENT]: Couldn't delete subscription %u. Failed with error %s. No further actions will be taken\n",
...
void CUA_ClientInformation::CUA_RemoteCallbackFunctions::readAsyncCallback(UA_Client *,
  ...
  if (UA_STATUSCODE_GOOD == paResponse->responseHeader.serviceResult) {
    if (paResponse->resultsSize == remoteCallHandle->mActionInfo.getNoOfNodePairs()) {
      ...
      DEVLOG_ERROR("[OPC UA CLIENT]: Reading for FB %s in client %s failed because the response size is %u but the FB "
...
void CUA_ClientInformation::CUA_RemoteCallbackFunctions::writeAsyncCallback(UA_Client *,
  ...
  if (UA_STATUSCODE_GOOD == paResponse->responseHeader.serviceResult) {
    if (paResponse->resultsSize == remoteCallHandle->mActionInfo.getNoOfNodePairs()) {
      ...
      DEVLOG_ERROR("[OPC UA CLIENT]: Writing for FB %s in client %s failed because the response size is %u but the FB "
    ...
    if (1 == response->resultsSize) {
      if (UA_STATUSCODE_GOOD == response->results[0].statusCode) {
        ...
        if (remoteCallHandle->mActionInfo.getLayer().getCommFB()->getNumRD() !=
          ...
          DEVLOG_ERROR("[OPC UA CLIENT]: Calling for FB %s in client %s failed because the number of RD connectors of "
      ...
      DEVLOG_ERROR(
          "[OPC UA CLIENT]: Calling for FB %s in client %s failed because the response size is %u, different from 1\n",
          remoteCallHandle->mActionInfo.getLayer().getCommFB()->getInstanceName(),
