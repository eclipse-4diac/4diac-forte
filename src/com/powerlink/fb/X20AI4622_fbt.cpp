...
DEFINE_FIRMWARE_FB(FORTE_X20AI4622, "X20AI4622"_STRID)

namespace {
  const auto cDataInputNames = std::array{"QI"_STRID, "CNID"_STRID, "MODID"_STRID};
  const auto cDataOutputNames =
      std::array{"QO"_STRID, "CNIDO"_STRID, "STATUS"_STRID, "AI01"_STRID, "AI02"_STRID, "AI03"_STRID, "AI04"_STRID};
...
FORTE_X20AI4622::FORTE_X20AI4622(const forte::core::StringId paInstanceNameId, forte::core::CFBContainer &paContainer) :
...
void FORTE_X20AI4622::writeOutputData(const TEventID paEIID) {
  switch (paEIID) {
    ...
    case scmEventCNFID: {
      ...
      writeData(cFBInterfaceSpec.getNumDIs() + 5, var_AI03, conn_AI03);
      writeData(cFBInterfaceSpec.getNumDIs() + 6, var_AI04, conn_AI04);
      writeData(cFBInterfaceSpec.getNumDIs() + 0, var_QO, conn_QO);
      break;
    }
    default: break;
