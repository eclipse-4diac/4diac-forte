...
CrcXSocketInterface::CrcXSocketInterface() {
  ...
  eRslt =
      TLR_POOL_CREATE(scmFortePoolName, cg_fortePoolMax, sizeof(FORTE_TCP_PACKET_T), &mForteResources.fortePoolHandle);
  if (eRslt != RX_OK) {
...
void CrcXSocketInterface::run() {
  ...
  while (isAlive()) {
    ...
    while (!mWaitingList.isEmpty()) {
      ...
      FORTE_TCP_PACKET_T *firstPacket =
          *(static_cast<CSinglyLinkedList<FORTE_TCP_PACKET_T *>::Iterator>(mWaitingList.begin()));
      TConnectionContainer::Iterator itEndConnection(mConnectionsList.end());
      bool deletePacket = true;
      for (TConnectionContainer::Iterator itRunnerConnection = mConnectionsList.begin();
           itRunnerConnection != itEndConnection;) {
...
CrcXSocketInterface::TSocketDescriptor CrcXSocketInterface::openTCPServerConnection(char *paIPAddr,
  ...
  if (CrcXSocketInterface::getInstance().isInitialized()) {
    ...
    if (0 != returnSocket) {
      RX_RESULT retVal =
          CrcXSocketInterface::getInstance().openConnection(paIPAddr, paPort, true, true, 0, returnSocket);
      if (RX_OK == retVal) {
        return returnSocket;
      } else {
...
CrcXSocketInterface::TSocketDescriptor CrcXSocketInterface::openTCPClientConnection(char *paIPAddr,
                                                                                    unsigned short paPort) {
  DEVLOG_INFO("CrcXSocketInterface: Opening TCP-Server connection at: %s:%d\n", paIPAddr, paPort);
  if (CrcXSocketInterface::getInstance().isInitialized()) {
    TSocketDescriptor returnSocket = CrcXSocketInterface::getInstance().socketDescriptorAlloc();
    if (0 != returnSocket) {
      RX_RESULT retVal =
          CrcXSocketInterface::getInstance().openConnection(paIPAddr, paPort, true, false, 0, returnSocket);
      if (RX_OK == retVal) {
        return returnSocket;
      } else {
        DEVLOG_ERROR("Error %d opening socket\n", retVal);
...
CrcXSocketInterface::TSocketDescriptor
CrcXSocketInterface::openUDPSendPort(char *paIPAddr, unsigned short paPort, TUDPDestAddr *mDestAddr) {
  DEVLOG_INFO("CrcXSocketInterface: Opening UDP sending connection at: %s:%d\n", paIPAddr, paPort);
  if (CrcXSocketInterface::getInstance().isInitialized()) {
    TSocketDescriptor returnSocket = CrcXSocketInterface::getInstance().socketDescriptorAlloc();
    if (0 != returnSocket) {
      RX_RESULT retVal =
          CrcXSocketInterface::getInstance().openConnection(paIPAddr, paPort, false, false, mDestAddr, returnSocket);
      if (RX_OK == retVal) {
        return returnSocket;
      } else {
        DEVLOG_ERROR("Error %d opening socket\n", retVal);
...
CrcXSocketInterface::TSocketDescriptor CrcXSocketInterface::openUDPReceivePort(char *paIPAddr, unsigned short paPort) {
  ...
  if (CrcXSocketInterface::getInstance().isInitialized()) {
    ...
    if (0 != returnSocket) {
      RX_RESULT retVal =
          CrcXSocketInterface::getInstance().openConnection(paIPAddr, paPort, false, true, 0, returnSocket);
      if (RX_OK == retVal) {
        return returnSocket;
      } else {
        DEVLOG_ERROR("Error %d opening socket\n", retVal);
...
RX_RESULT CrcXSocketInterface::close(TSocketDescriptor paSockD) {
  if (scmInvalidSocketDescriptor != paSockD) {
    ...
    return CrcXSocketInterface::getInstance().sendPacketToTCP(
        paSockD->socketNumber, TCPIP_DATA_TCP_UDP_CMD_CLOSE_REQ_SIZE, TCPIP_TCP_UDP_CMD_CLOSE_REQ, &tDataClose,
...
RX_RESULT CrcXSocketInterface::openConnection(char *paIPAddr,
                                              unsigned short paPort,
                                              bool isTCP,
                                              bool isServer,
                                              TUDPDestAddr *mDestAddr,
                                              TSocketDescriptor &pa_destSocket) {
  ...
  retVal = sendPacketToTCP(0, TCPIP_DATA_TCP_UDP_CMD_OPEN_REQ_SIZE, TCPIP_TCP_UDP_CMD_OPEN_REQ, &tDataOpen,
                           sizeof(TCPIP_DATA_TCP_UDP_CMD_OPEN_REQ_T)); // send packet to open a socket
  if (RX_OK == retVal) {
    TCPIP_PACKET_TCP_UDP_CMD_OPEN_CNF_T *cnfPacket;
    retVal = waitPacket(TCPIP_TCP_UDP_CMD_OPEN_CNF, (FORTE_TCP_PACKET_T **) &cnfPacket,
                        RX_INFINITE); // wait for CNF packet which indicates the result of the request
    if (RX_OK == retVal) {
      if (RX_OK == cnfPacket->tHead.ulSta) {
        if (!isTCP) {
          ...
          if (0xE0000000 <= ipAddress && 0xEFFFFFFF >= ipAddress) {
            ...
            retVal = sendPacketToTCP(
                socketNumber, TCPIP_DATA_TCP_UDP_CMD_SET_SOCK_OPTION_REQ_SIZE, TCPIP_TCP_UDP_CMD_SET_SOCK_OPTION_REQ,
                &setOpt, sizeof(TCPIP_DATA_TCP_UDP_CMD_SET_SOCK_OPTION_REQ_T)); // send packet to open a socket
            if (RX_OK == retVal) {
              TCPIP_PACKET_TCP_UDP_CMD_SET_SOCK_OPTION_CNF_T *setOptCnf;
              retVal = waitPacket(TCPIP_TCP_UDP_CMD_SET_SOCK_OPTION_CNF, (FORTE_TCP_PACKET_T **) &setOptCnf,
                                  RX_INFINITE); // wait for CNF packet which indicates the result of the request
              if (RX_OK == retVal) {
          ...
          if (isServer) { // TCP server
            ...
            retVal = sendPacketToTCP(cnfPacket->tHead.ulDestId, TCPIP_DATA_TCP_CMD_WAIT_CONNECT_REQ_SIZE,
                                     TCPIP_TCP_CMD_WAIT_CONNECT_REQ, &tDataWait,
            ...
            tDataConnect.ulIpAddr = CrcXSocketInterface::getInstance().stringIpToInt(
                paIPAddr); /* IP address of remote server to connect to */
            if (0 != tDataConnect.ulIpAddr) {
              ...
              retVal =
                  sendPacketToTCP(cnfPacket->tHead.ulDestId, TCPIP_DATA_TCP_CMD_CONNECT_REQ_SIZE,
                                  TCPIP_TCP_CMD_CONNECT_REQ, &tDataConnect, sizeof(TCPIP_DATA_TCP_CMD_CONNECT_REQ_T));
              if (RX_OK == retVal) {
...
RX_RESULT
CrcXSocketInterface::sendPacketToTCP(UINT32 pa_destId, UINT32 paLen, UINT32 paCmd, void *paData, UINT32 pa_dataLength) {
  ...
  if (RX_OK == retVal) {
    ...
    retVal = TLR_QUE_SENDPACKET_FIFO(mForteResources.tcpQueueLink, ptPck,
                                     100); // rX_QueSendPacket((void*) mForteResources.tcpQueueLink.hQue, ptPck, 100);
    if (RX_OK != retVal) {
...
RX_RESULT CrcXSocketInterface::waitPacket(UINT32 pa_command, FORTE_TCP_PACKET_T **pa_packetResult, UINT pa_timeout) {
  ...
  do {
    retVal = TLR_QUE_WAITFORPACKET(mForteResources.forteQueueHandle, &localPacket,
                                   pa_timeout); // pa_timeout);rX_QueWaitForPacket((void*)mForteResources.forteQueueHandle,
                                                // (void**) &localPacket, 100); //
    if (0 != localPacket) {
  ...
  if (0 != pa_packetResult) {
    switch (pa_packetResult->tHead.ulCmd) {
      ...
      case TCPIP_TCP_UDP_CMD_SHUTDOWN_IND: {
        // close all sockets. TODO: The caller will never get notice that the socket was closed. Maybe reopen the socket
        // when the CNF arrives
        ...
        sendPacketToTCP(0, TCPIP_DATA_TCP_UDP_CMD_CLOSE_ALL_REQ_SIZE, TCPIP_TCP_UDP_CMD_CLOSE_ALL_REQ, &tDataCloseAll,
...
RX_RESULT CrcXSocketInterface::sendData(TSocketDescriptor paSockD,
                                        char *paData,
                                        unsigned int paSize,
                                        bool paTCP,
  ...
  *pa_result = -1;
  ...
  if (scmInvalidSocketDescriptor != paSockD) {
    if (paSockD->accepted) {
      ...
      if (paTCP) {
        ((TCPIP_DATA_TCP_CMD_SEND_REQ_T *) pa_PacketData)->ulOptions =
            TCP_SEND_OPT_PUSH; /* TCP_SEND_OPT_PUSH(0x00000001) Push flag: If set, the stack send the data immediate*/
      } else {
      ...
      while (0 != paSize) {
        if (paTCP) {
          *pa_result = (paSize > 1460) ? 1460 : paSize;
          ...
          retVal = sendPacketToTCP(paSockD->socketNumber, TCPIP_DATA_TCP_CMD_SEND_REQ_SIZE + *pa_result,
                                   TCPIP_TCP_CMD_SEND_REQ, pa_PacketData, sizeof(UINT32) + *pa_result);
        } else {
          *pa_result = (paSize > 1472) ? 1472 : paSize;
          TLR_MEMCPY(&(((TCPIP_DATA_UDP_CMD_SEND_REQ_T *) pa_PacketData)->abData[0]), paData, *pa_result);
          retVal = sendPacketToTCP(paSockD->socketNumber, TCPIP_DATA_UDP_CMD_SEND_REQ_SIZE + *pa_result,
                                   TCPIP_UDP_CMD_SEND_REQ, pa_PacketData, sizeof(UINT32) * 3 + *pa_result);
        }
...
int CrcXSocketInterface::sendDataOnUDP(TSocketDescriptor paSockD,
                                       TUDPDestAddr *paDestAddr,
                                       char *paData,
                                       unsigned int paSize) {
  int bytesSent = -1;
...
RX_RESULT CrcXSocketInterface::accept(TSocketDescriptor pa_listeningSocketDesc, TSocketDescriptor &pa_destSocket) {
  ...
  if (mListeningSocketDescriptor == pa_listeningSocketDesc) {
    if (scmInvalidSocketDescriptor != pa_destSocket) {
      ...
      retVal = CrcXSocketInterface::getInstance().openConnection(
          0, static_cast<unsigned short>(pa_listeningSocketDesc->port), true, true, 0, pa_listeningSocketDesc);
      if (RX_OK != retVal) {
        DEVLOG_ERROR("Error %d opening socket\n", retVal);
      }
    } else {
...
RX_RESULT CrcXSocketInterface::receiveData(
    TSocketDescriptor paSockD, bool, char *paData, unsigned int paBufSize, int *pa_receivedBytes) {
  RX_RESULT retVal = RX_OK;
  *pa_receivedBytes = -1;
    ...
    if (TCPIP_TCP_UDP_CMD_RECEIVE_STOP_IND == paSockD->packetReceived->tHead.ulCmd) {
      *pa_receivedBytes = 0;
      ...
      if (paBufSize >= (paSockD->packetReceived->tHead.ulLen - TCPIP_DATA_TCP_UDP_CMD_RECEIVE_IND_SIZE)) {
        TLR_MEMCPY(paData, &paSockD->packetReceived->tRcvInd.tData.abData[0],
                   (paSockD->packetReceived->tHead.ulLen - TCPIP_DATA_TCP_UDP_CMD_RECEIVE_IND_SIZE));
        *pa_receivedBytes = (paSockD->packetReceived->tHead.ulLen - TCPIP_DATA_TCP_UDP_CMD_RECEIVE_IND_SIZE);
        ...
        TLR_MEMCPY(&buffer[0], &paSockD->packetReceived->tRcvInd.tData.abData[paBufSize],
                   (paSockD->packetReceived->tHead.ulLen - TCPIP_DATA_TCP_UDP_CMD_RECEIVE_IND_SIZE) - paBufSize);
        TLR_MEMCPY(&paSockD->packetReceived->tRcvInd.tData.abData[0], &buffer[0],
                   (paSockD->packetReceived->tHead.ulLen - TCPIP_DATA_TCP_UDP_CMD_RECEIVE_IND_SIZE) - paBufSize);
