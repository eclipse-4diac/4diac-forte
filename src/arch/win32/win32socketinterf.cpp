...
CWin32SocketInterface::TSocketDescriptor CWin32SocketInterface::openTCPServerConnection(char *paIPAddr,
  ...
  if (INVALID_SOCKET != nSocket) {
    ...
    if (0 == bind(nSocket, (struct sockaddr *) &stSockAddr, sizeof(struct sockaddr))) {
      if (SOCKET_ERROR == listen(nSocket, 1)) { // for the classic IEC 61499 server only one connection at the same time
...
int CWin32SocketInterface::sendDataOnUDP(TSocketDescriptor paSockD,
  ...
  while (0 < nToSend) {
    // TODO: check if open connection (socket might be closed by peer)
    nRetVal =
        static_cast<int>(sendto(paSockD, paData, nToSend, 0, (struct sockaddr *) paDestAddr, sizeof(struct sockaddr)));
    if (nRetVal <= 0) {
      int nLastError = WSAGetLastError();
      LPSTR pacErrorMessage = getErrorMessage(nLastError);
