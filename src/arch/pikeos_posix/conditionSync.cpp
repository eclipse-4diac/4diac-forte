/*******************************************************************************
 * Copyright (c) 2016, 2018 Johannes Messmer (admin@jomess.com), SYSGO AG
 ...
 *    Agostino Mascitti - Adaption to PikeOS
 *******************************************************************************/
...
ConditionSync::ConditionSync() {
  ...
  // PikeOS only implements the CLOCK_REALTIME clock. In PikeOS time cannot be changed at run-time. So it's equivalent to CLOCK_MONOTONIC
  ...
  #elif PIKEOS
  pthread_mutexattr_settype(&mutexAttr, PTHREAD_MUTEX_RECURSIVE); // PikeOS has PTHREAD_MUTEX_RECURSIVE
  #else
