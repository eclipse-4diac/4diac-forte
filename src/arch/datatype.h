...
const unsigned int cgIdentifierLength = 32; //!< This value holds the length an identifier may have on order that it is
                                            //!< unique. Identifiers may be FB-Type Names, FB-Names, var names,...

typedef char TIdentifier[cgIdentifierLength +
                         1]; //!< Type to hold an identifier during runtime. The +1 is for holding a trailing \\0

typedef size_t TPortId; //!< Type for holding an optimized FORTE internal port number.

constexpr TPortId cgInvalidPortId =
    65535; //!< \ingroup CORE constant defining the max possible number of an FB Port (event or data, in or output)
