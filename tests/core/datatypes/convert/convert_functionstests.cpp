...
BOOST_AUTO_TEST_SUITE(convert_functions)
  ...
  BOOST_AUTO_TEST_CASE(CONVERT_TIME_TO_MS_LREAL) {
    CIEC_TIME time("T#2d5h43m12s44ms27us31ns");
    CIEC_LREAL timeInS;
    timeInS = TIME_IN_MS_TO_LREAL(time);
    BOOST_TEST(
      timeInS == static_cast<CIEC_LREAL>(INT64_C(172800000000000) + INT64_C(18000000000000) + INT64_C(2580000000000) + INT64_C(12000000000) + INT64_C(44000000) + INT64_C(27000) + INT64_C(31)) / static_cast<CIEC_LREAL>(1E6));
  }
