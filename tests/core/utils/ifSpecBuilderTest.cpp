...
static bool operator==(const SAdapterInstanceDef &a, const SAdapterInstanceDef &b) {
  return a.mAdapterTypeNameID == b.mAdapterTypeNameID && a.mAdapterNameID == b.mAdapterNameID && a.mIsPlug == b.mIsPlug;
}
...
void test_data(CIfSpecBuilder &uut,
               TPortId &n,
               const CStringDictionary::TStringId *&f,
               const CStringDictionary::TStringId *&t,
               CStringDictionary::TStringId dataTypeId = CStringDictionary::scmInvalidStringId) {
  build(uut);
  BOOST_REQUIRE_EQUAL(n, 2);
  BOOST_REQUIRE_NE(f, nullptr);
...
void test_bind(const TDataIOID *&w, const TForteInt16 *&i) {
  static constexpr std::array<TDataIOID, 5> tw = {0, 2, CFunctionBlock::scmWithListDelimiter, 1,
                                                  CFunctionBlock::scmWithListDelimiter};
