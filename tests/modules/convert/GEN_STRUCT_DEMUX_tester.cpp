...
BOOST_AUTO_TEST_CASE(missingDataType) {
  CFunctionBlock *fb = forte::core::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1UnknownType),
...
BOOST_AUTO_TEST_CASE(wrongDataType) {
  CFunctionBlock *fb =
      forte::core::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1STRING), CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr == fb);

  fb = forte::core::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1REAL), CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr == fb);
}

BOOST_AUTO_TEST_CASE(emptyStruct) {
  CFunctionBlock *fb = forte::core::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1Struct_Muxer_Test_Struct_3),
                                             CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr == fb);
}

BOOST_AUTO_TEST_CASE(bigStruct) {
  CFunctionBlock *fb = forte::core::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1Struct_Muxer_Test_Struct_4),
                                             CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr != fb);
  delete fb;
}
