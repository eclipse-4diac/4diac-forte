...
BOOST_AUTO_TEST_CASE(missingDataType) {
  CFunctionBlock *fb =
      CTypeLib::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1UnknownType), CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr == fb);
}

BOOST_AUTO_TEST_CASE(wrongDataType) {
  CFunctionBlock *fb =
      CTypeLib::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1STRING), CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr == fb);
...
BOOST_AUTO_TEST_CASE(emptyStruct) {
  CFunctionBlock *fb = CTypeLib::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1Struct_Muxer_Test_Struct_3),
                                          CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr == fb);
}

BOOST_AUTO_TEST_CASE(bigStruct) {
  CFunctionBlock *fb = CTypeLib::createFB(STRID(InstanceName), STRID(STRUCT_DEMUX_1Struct_Muxer_Test_Struct_4),
                                          CFBContainerMock::smDefaultFBContMock);
  BOOST_CHECK(nullptr != fb);
  delete fb;
}
