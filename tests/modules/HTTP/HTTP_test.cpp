...
BOOST_AUTO_TEST_CASE(PUTPOST_test) {
  ...
  const char *postResultValid1 = "POST /?key1=val1;key2=val2 HTTP/1.1\r\nHost: 0.0.0.0:80\r\nContent-Type: "
                                 "text/xml\r\nContent-Length: 19\r\n\r\ncontent of the body";
  forte::com_infra::CHttpParser::createPutPostRequest(dest, host, port, path, auth, params, body, contentType,
                                                      forte::com_infra::CHttpComLayer::e_PUT);
  BOOST_TEST(dest == putResultValid1);
  forte::com_infra::CHttpParser::createPutPostRequest(dest, host, port, path, auth, params, body, contentType,
                                                      forte::com_infra::CHttpComLayer::e_POST);
...
BOOST_AUTO_TEST_CASE(parseGetRequest_test) {
  ...
  BOOST_TEST(false ==
             forte::com_infra::CHttpParser::parseGetRequest(path, parameterNames, parameterValues, invalidPath.data()));
}
