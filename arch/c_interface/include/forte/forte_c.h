...
/**
 * \brief Initializes the architecture. Prepare all resources needed by the Forte's instances. Must be called once
 * before the first Forte instance is started
 * @param argc Number of arguments in arg
 * @param argv Arguments
 * @return 0 if no error occurred, other values otherwise. Any error value is provided by the architecture and not by
 * forte
 */
FORTE_SHARED_PREFIX int forteGlobalInitialize(int argc, char *argv[]);

/**
 * \brief Deinitializes the architecture. Frees all resources used by Forte's instances. Must be called after the last
 * instance is ended
 * @return 0 if no error occurred, other values otherwise.  Any error value is provided by the architecture and not by
 * forte
 */
