...
#include "forte/iec61131/counters/FB_CTUD_fbt.h"
...
FORTE_FB_CTUD::FORTE_FB_CTUD(const forte::StringId paInstanceNameId, forte::CFBContainer &paContainer) :
...
void FORTE_FB_CTUD::setInitialValues() {
...
void FORTE_FB_CTUD::executeEvent(const TEventID paEIID, CEventChainExecutionThread *const paECET) {
  switch (paEIID) {
...
void FORTE_FB_CTUD::enterStateREQ(CEventChainExecutionThread *const paECET) {
  alg_REQ();
  sendOutputEvent(scmEventCNFID, paECET);
}

void FORTE_FB_CTUD::readInputData(const TEventID paEIID) {
  switch (paEIID) {
    case scmEventREQID: {
...
void FORTE_FB_CTUD::writeOutputData(const TEventID paEIID) {
  switch (paEIID) {
    case scmEventCNFID: {
...
CIEC_ANY *FORTE_FB_CTUD::getDI(const size_t paIndex) {
  switch (paIndex) {
...
CIEC_ANY *FORTE_FB_CTUD::getDO(const size_t paIndex) {
  switch (paIndex) {
...
CEventConnection *FORTE_FB_CTUD::getEOConUnchecked(const TPortId paIndex) {
  switch (paIndex) {
...
CDataConnection **FORTE_FB_CTUD::getDIConUnchecked(const TPortId paIndex) {
  switch (paIndex) {
...
CDataConnection *FORTE_FB_CTUD::getDOConUnchecked(const TPortId paIndex) {
  switch (paIndex) {
...
#line 9 "FB_CTUD.fbt"
        if (func_AND<CIEC_BOOL>(var_CU, func_LT(var_CV, 32767_INT))) {
#line 10 "FB_CTUD.fbt"
          var_CV = func_ADD<CIEC_INT>(var_CV, 1_INT);
        } else if (func_AND<CIEC_BOOL>(var_CD, func_GT(var_CV, -32768_INT))) {
#line 12 "FB_CTUD.fbt"
          var_CV = func_SUB<CIEC_INT>(var_CV, 1_INT);
        }
      }
    }
  }
