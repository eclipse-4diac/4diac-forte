...
namespace CONMELEON {
   ...
   *  This approach is  slower than the direct memory access of the GPIO chip registers, but it is available almost on
   * every Linux based embedded device without changes SYSFS files follow a given file path structure, please note that
   * you normally need root privileges to access the SYSFS files. to use a GPIO pin you need to write its number to the
   * /sys/class/gpio/export file to stop using a pin you need to write its number to the /sys/class/gpio/unexport file
   *  to set the direction of pin 4 (input or output) you need to write 'in' or 'out' to /sys/class/gpio/gpio4/direction
   * file to set the value of pin 15 (high or low) you need to write '0' or '1' to /sys/class/gpio/gpio15/value file
  ...
  class CGpioPin : private CUncopyable {
    public:
       ...
       *
       * The constructor checks for valid pin numbers ( > 0 ) and writes the pin number to /sys/class/gpio/export to be
       * able to use the pin
       */
      ...
      bool isAvailable() const {
        return (m_Valid && (m_State == unused));
      }

      /*! \fn bool isInput()
       * \brief returns true if the pin is configured as an input or false if not
       *
       * The pin is configured as an input or not, this function also returns false if the pin is undefined (default
       * setting of the constructor)
       */
      bool isInput() const {
        return (m_Direction == input);
      }

      /*! \fn bool isOutput()
       * \brief returns true if the pin is configured as an output or false if not
       *
       * The pin is configured as an output or not, this function also returns false if the pin is undefined (default
       * setting of the constructor)
       */
      bool isOutput() const {
       ...
       *
       * Read the current pin value from the /sys/class/gpio/gpioxx/value file, this only works if the pin has been
       ...
       * \param bValue
       *
       * Write bValue to the /sys/class/gpio/gpioxx/value file, this only works if the pin has been configured as an
       * output SYSFS expects "1" for TRUE and "0" for FALSE
       */
