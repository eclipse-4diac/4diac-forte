/*******************************************************************************
 * Copyright (c) 2006 ACIN, Profactor GmbH, and others
...
namespace forte::eclipse4diac::rtevents {
  class FORTE_RT_E_CYCLE final : public CEventSourceFB {
    ...
    public:
      ...
      void evt_START(const CIEC_TIME &pa_DT, const CIEC_TIME &pa_Deadline, const CIEC_TIME &pa_WCET) {
        var_DT = pa_DT;
        var_Deadline = pa_Deadline;
        var_WCET = pa_WCET;
        receiveInputEvent(scmEventSTARTID, nullptr);
      }
      void evt_STOP(const CIEC_TIME &pa_DT, const CIEC_TIME &pa_Deadline, const CIEC_TIME &pa_WCET) {
        var_DT = pa_DT;
        var_Deadline = pa_Deadline;
        var_WCET = pa_WCET;
        receiveInputEvent(scmEventSTOPID, nullptr);
      }
      void operator()(const CIEC_TIME &pa_DT, const CIEC_TIME &pa_Deadline, const CIEC_TIME &pa_WCET) {
        evt_START(pa_DT, pa_Deadline, pa_WCET);
      }
  };
} // namespace forte::eclipse4diac::rtevents
