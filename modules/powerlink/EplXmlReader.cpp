...
void CEplXmlReader::readXmlFile(const char *paFileName) {
  ...
  if (xmlDoc.LoadFile()) {
    ...
    for (TiXmlNode *processImage = appProcess->ToElement()->FirstChild("ProcessImage"); processImage != nullptr;
         processImage = processImage->NextSibling("ProcessImage")) {
...
void CEplXmlReader::createProcImageOut(TiXmlNode *paProcessImage) {
  ...
  for (channel = paProcessImage->FirstChild("Channel"); channel != nullptr; channel = channel->NextSibling("Channel")) {
    ...
    } else if (mModuleListOut.moduleNameExist(
                   ioName)) { // IO name exist => this must be a new module with same brand as another
      currentModuleNr++;
      strcpy(currentModuleId, modId);
      currentIoNr = 0;
    ...
    DEVLOG_DEBUG("[powerlink] << %d, %d, %d, %d, %d, %d, %d\n", currentCnId, currentModuleNr, ioName, currentIoNr,
                 dSize, piOffset, bitOffset);
...
void CEplXmlReader::createProcImageIn(TiXmlNode *paProcessImage) {
  ...
  for (channel = paProcessImage->FirstChild("Channel"); channel != nullptr; channel = channel->NextSibling("Channel")) {
    ...
    } else if (mModuleListIn.moduleNameExist(
                   ioName)) { // IO name exist => this must be a new module with same brand as another
