...
namespace forte::iec61499::system {
  ...
  EMGMResponse OPCUA_MGR::addMgmtObjectInstance() {
    ...
    UA_StatusCode status = UA_Server_addObjectNode(
        mUaHandler.getUAServer(), UA_NODEID_STRING(smNamespaces[1], smMgmtName),
        UA_NODEID_NUMERIC(smNamespaces[0], UA_NS0ID_OBJECTSFOLDER),
        UA_NODEID_NUMERIC(smNamespaces[0], UA_NS0ID_ORGANIZES), UA_QUALIFIEDNAME(smNamespaces[1], smMgmtName),
        mMgmtTypeId, oAttr, nullptr, &mMgmtNodeId);
  ...
  EMGMResponse
  OPCUA_MGR::addResourceObjectInstance(char *paResourceName, UA_NodeId &paNodeId, UA_NodeId paParentNodeId) {
    EMGMResponse eRetVal = EMGMResponse::Ready;
    UA_ObjectAttributes oAttr = UA_ObjectAttributes_default;
    oAttr.displayName = UA_LOCALIZEDTEXT(smEmptyLocale, paResourceName);

    UA_StatusCode status = UA_Server_addObjectNode(mUaHandler.getUAServer(), UA_NODEID_NULL, paParentNodeId,
                                                   UA_NODEID_NUMERIC(smNamespaces[0], UA_NS0ID_ORGANIZES),
                                                   UA_QUALIFIEDNAME(smNamespaces[1], paResourceName), mResourceTypeId,
                                                   oAttr, paResourceName, &paNodeId);
    if (status != UA_STATUSCODE_GOOD) {
      return EMGMResponse::InvalidState;
    }
    return eRetVal;
  ...
  EMGMResponse OPCUA_MGR::addCreateResourceMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smCreateResMethodName, mMgmtTypeId, createResAttr, inputArguments, 2,
                         &outputArgument, 1, &onCreateResource);
  }
  ...
  EMGMResponse OPCUA_MGR::addCreateConnectionMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smCreateConnMethodName, mResourceTypeId, createConnAttr, inputArguments, 2, nullptr,
                         0, &onCreateConnection);
  }
  ...
  EMGMResponse OPCUA_MGR::addStopDeviceMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smStopDevMethodName, mMgmtTypeId, stopDevAttr, nullptr, 0, nullptr, 0,
                         &onStopDevice);
  }
  ...
  EMGMResponse OPCUA_MGR::addKillDeviceMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smKillDevMethodName, mMgmtTypeId, killDevAttr, nullptr, 0, nullptr, 0,
                         &onKillDevice);
  }
  ...
  EMGMResponse OPCUA_MGR::addDeleteConnectionMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smDeleteConnMethodName, mResourceTypeId, deleteConnAttr, inputArguments, 2, nullptr,
                         0, &onDeleteConnection);
  }
  ...
  EMGMResponse OPCUA_MGR::addReadWatchesMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smReadWatchesMethodName, mMgmtTypeId, readWatchesAttr, nullptr, 0, &outputArgument,
                         1, &onReadWatches);
  }
  ...
  EMGMResponse OPCUA_MGR::addRemoveWatchMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smRemoveWatchMethodName, mResourceTypeId, removeWatchAttr, &inputArgument, 1,
                         nullptr, 0, &onRemoveWatch);
  }
  ...
  EMGMResponse OPCUA_MGR::addTriggerEventMethod(UA_Server *paServer) {
    ...
    UA_MethodAttributes triggerEventAttr =
        createAttribute(smTriggerEventAttrDisplayName, smTriggerEventAttrDescription);
  ...
  EMGMResponse OPCUA_MGR::addForceValueMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smForceValueMethodName, mResourceTypeId, forceValueAttr, inputArguments, 2, nullptr,
                         0, &onForceValue);
  }
  ...
  EMGMResponse OPCUA_MGR::addClearForceMethod(UA_Server *paServer) {
    ...
    return addMethodNode(paServer, smClearForceMethodName, mResourceTypeId, clearForceAttr, &inputArgument, 1, nullptr,
                         0, &onClearForce);
  }
  ...
  OPCUA_MGR::addExtraMethods(UA_Server *paServer, UA_NodeId paParentNodeId, std::vector<MethodInformation> &paMethods) {
    for (auto &method : paMethods) {
      ...
      if (auto result =
              addMethodNode(paServer, method.mMethodName.data(), paParentNodeId, attributes, method.mInArguments.data(),
                            method.mInArguments.size(), method.mOutArguments.data(), method.mOutArguments.size(),
                            method.mCallback, method.mNodeContext);
