#!/bin/bash
echo "----------------------------------------------------------------------------"
echo " Automatically set up development environment for FreeRTOS-platform on ESP32"
echo "----------------------------------------------------------------------------"
echo ""
...
if [ -d "$forte_bin_dir" ]; then
  ...
  cmake -G "Unix Makefiles" \
    -DCMAKE_TOOLCHAIN_FILE="${FORTE_BUILDSUPPORT_DIRECTORY}/toolchains/toolchain-esp32.cmake" \
    ...
    -DCMAKE_INSTALL_PREFIX=../../../Application/lib/esp32 \
    -DCMAKE_CXX_STANDARD_INCLUDE_DIRECTORIES="${PWD}/../../../Application/build_ESP32_dual_can/config/" \
