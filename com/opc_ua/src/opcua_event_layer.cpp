...
COPC_UA_Event_Layer::addNewEventType(UA_Server *paServer, UA_NodeId &paEventType, std::string paEventTypeName) {
  ...
  UA_StatusCode status = UA_Server_addObjectTypeNode(
      paServer, UA_NODEID_NULL, UA_NODEID_NUMERIC(0, UA_NS0ID_BASEEVENTTYPE), UA_NODEID_NUMERIC(0, UA_NS0ID_HASSUBTYPE),
      UA_QUALIFIEDNAME(0, eventTypeName.data()), attr, NULL, &paEventType);
  if (status != UA_STATUSCODE_GOOD) {
...
UA_StatusCode COPC_UA_Event_Layer::writeTimeAndSourceProperty(UA_Server *paServer, UA_NodeId &paNodeId) {
  ...
  UA_StatusCode status =
      UA_Server_writeObjectProperty_scalar(paServer, paNodeId, UA_QUALIFIEDNAME(scmServerNSIndex, smEventTimeProperty),
