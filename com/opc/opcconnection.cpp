...
COpcConnection::COpcConnection(const std::string &paHost,
                               const std::string &paServerName,
                               COpcEventHandler *paEventHandler) :
    mGroupCount(0),
    mConnectionState(e_Disconnected),
    mHost(paHost),
    mServerName(paServerName),
    mConnected(false),
    mBlockingConnect(false),
    mEventHandler(paEventHandler) {
...
void COpcConnection::addGroup(const std::string &paGroupName,
...
bool COpcConnection::ifLetEventPass(EOpcConnectionEvents paEvent, const std::string &paGroupName) {
  ...
  switch (mConnectionState) {
    case e_Disconnected: return (e_Connect == paEvent);

    case e_Connecting: return (e_Connect == paEvent && !ifInGroupList(paGroupName));
    ...
    case e_Disconnecting: return (e_Disconnect == paEvent);
  }
  return false;
}
...
int COpcConnection::send_connect(const std::string &paGroupName,
  ...
  if (ifLetEventPass(e_Connect, paGroupName)) {
    ...
    mEventHandler->sendCommand(
        new CCmd_AddConnection(mOpcConnectionImpl, paGroupName, paReqUpdateRate, paDeadBand, paNewItems));
  } else {
...
void COpcConnection::response_connect(bool paConnectionState, const std::string &paGroupName) {
  ...
  if (!mBlockingConnect && paConnectionState) {
    for (TOpcGroupMapList::iterator it = mOpcGroupMapList.begin(); it != mOpcGroupMapList.end(); ++it) {
  ...
  if (!paItemDataList.empty()) {
    ...
    for (size_t i = 0; i < paItemDataList.size(); i++) {
      DEVLOG_INFO("deleting item from onchange itemlist[%s:%s]\n", paGroupName.c_str(),
                  paItemDataList[i]->mItemName.c_str());
