...
if [ -d "$forte_bin_dir" ]; then
  ...
  cmake -G "Unix Makefiles" \
    -DCMAKE_TOOLCHAIN_FILE="${FORTE_BUILDSUPPORT_DIRECTORY}/toolchains/toolchain-esp32-datapanel.cmake" \
    ...
    -DFORTE_MODULE_ISOBUS=ON \
    -DFORTE_IO=ON \
    -DFORTE_IO_COMMON_IO=ON \
    -DFORTE_MODULE_ESP32=ON \
    -DFORTE_MODULE_signalprocessing=ON \
    -DFORTE_MODULE_DATAPANEL=ON \
    -DFORTE_MODULE_BLINKMARINE=ON \
