...
message(STATUS "FORTE_MODULE_DIR: " ${FORTE_MODULE_DIR})
...
FILE(GLOB_RECURSE COM_FILES_FOUND "${FORTE_COM_DIR}/*CMakeLists.txt")
message(STATUS "FORTE_COM_DIR: " ${FORTE_COM_DIR})
FOREACH(FILE ${COM_FILES_FOUND})
  string(REPLACE "${FORTE_COM_DIR}" "" MODULE_NAME ${FILE})
  STRING(REGEX MATCH "^/([^/]*)/CMakeLists.txt$" MODULE_NAME ${MODULE_NAME})
  IF(MODULE_NAME)
    string(REPLACE "/CMakeLists.txt" "" MODULE_NAME "${MODULE_NAME}")    
    string(REPLACE "/" "" MODULE_NAME "${MODULE_NAME}")
    LIST(APPEND COM_LIST ${MODULE_NAME})
  ENDIF(MODULE_NAME)
ENDFOREACH(FILE)   
SET(WRITE_FILE "")
FOREACH(MODULE ${COM_LIST})
  SET(WRITE_FILE "${WRITE_FILE}SET(SOURCE_GROUP_BACKUP \"\${SOURCE_GROUP}\")\n")
  SET(WRITE_FILE "${WRITE_FILE}SET(SOURCE_GROUP \"\${SOURCE_GROUP}/${MODULE}\")\n")
  SET(WRITE_FILE "${WRITE_FILE}ADD_SUBDIRECTORY(\"\${FORTE_MODULE_DIR}${MODULE}\")\n")
  SET(WRITE_FILE "${WRITE_FILE}SET(SOURCE_GROUP \"\${SOURCE_GROUP_BACKUP}\")\n")
ENDFOREACH(MODULE)
file(WRITE ${CMAKE_BINARY_DIR}/src/com/CMakeLists_new.txt ${WRITE_FILE})
...
execute_process( COMMAND ${CMAKE_COMMAND} -E compare_files ${CMAKE_BINARY_DIR}/src/com/CMakeLists_new.txt ${CMAKE_BINARY_DIR}/src/com/CMakeLists.txt RESULT_VARIABLE test_not_successful OUTPUT_QUIET ERROR_QUIET )
...
IF(test_not_successful)
  ...
  file(REMOVE ${CMAKE_BINARY_DIR}/src/com/CMakeLists.txt)
  file(RENAME ${CMAKE_BINARY_DIR}/src/com/CMakeLists_new.txt ${CMAKE_BINARY_DIR}/src/com/CMakeLists.txt)
ENDIF( test_not_successful)  
file(REMOVE ${CMAKE_BINARY_DIR}/src/modules/CMakeLists_new.txt) 
FILE(REMOVE ${CMAKE_BINARY_DIR}/src/com/CMakeLists_new.txt)