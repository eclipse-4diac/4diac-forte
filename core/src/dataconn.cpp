...
EMGMResponse CDataConnection::connect(CFunctionBlock &paDstFB, const std::span<const forte::StringId> paDstPortNameId) {
  ...
  switch (getValue().getDataTypeID()) {
    ...
    case CIEC_ANY::e_STRUCT:
      if (CIEC_ANY *member = static_cast<CIEC_STRUCT &>(getValue()).getMemberNamed(paSrcNameList); member) {
        return make_delegating<forte::internal::CMemberDataConnection>(getSourceId().getFB(), getSourceId().getPortId(),
