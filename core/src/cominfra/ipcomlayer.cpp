...
EComResponse CIPComLayer::sendData(void *paData, unsigned int paSize) {
  ...
  if ((nullptr != mFb) && (CIPComSocketHandler::scmInvalidSocketDescriptor != mSocketID)) {
    switch (mFb->getComServiceType()) {
      case e_Server:
        if (0 >= CIPComSocketHandler::sendDataOnTCP(mSocketID, static_cast<char *>(paData), paSize)) {
      ...
      case e_Client:
        ...
        break;
      case e_Publisher:
        if (0 >= CIPComSocketHandler::sendDataOnUDP(mSocketID, &mDestAddr, static_cast<char *>(paData), paSize)) {
          eRetVal = e_InitTerminated;
        }
        break;
      case e_Subscriber:
...
EComResponse CIPComLayer::recvData(const void *paData, unsigned int) {
  ...
  switch (mConnectionState) {
    ...
    case e_Connected:
      ...
      } else if (mListeningID == *(static_cast<const CIPComSocketHandler::TSocketDescriptor *>(paData))) {
...
EComResponse CIPComLayer::openConnection(char *paLayerParameter) {
  ...
  if (nullptr != acPort) {
    *acPort = '\0';
    ...
    switch (mFb->getComServiceType()) {
      case e_Server:
        nSockDes = mListeningID = CIPComSocketHandler::openTCPServerConnection(paLayerParameter, nPort);
        mConnectionState = e_Listening;
        break;
      case e_Client:
        nSockDes = mSocketID = CIPComSocketHandler::openTCPClientConnection(paLayerParameter, nPort);
        break;
      case e_Publisher:
        nSockDes = mSocketID = CIPComSocketHandler::openUDPSendPort(paLayerParameter, nPort, &mDestAddr, acInterface);
        break;
      case e_Subscriber:
        nSockDes = mSocketID = CIPComSocketHandler::openUDPReceivePort(paLayerParameter, nPort, acInterface);
        break;
    }
...
void CIPComLayer::handledConnectedDataRecv() {
  ...
  if (CIPComSocketHandler::scmInvalidSocketDescriptor != mSocketID) {
    ...
    switch (mFb->getComServiceType()) {
      case e_Server:
      case e_Client:
        nRetVal = CIPComSocketHandler::receiveDataFromTCP(mSocketID, &mRecvBuffer[mBufFillSize],
      ...
      case e_Publisher:
        ...
        break;
      case e_Subscriber:
        nRetVal = CIPComSocketHandler::receiveDataFromUDP(mSocketID, &mRecvBuffer[mBufFillSize],
                                                          cgIPLayerRecvBufferSize - mBufFillSize);
        break;
    }
    switch (nRetVal) {
