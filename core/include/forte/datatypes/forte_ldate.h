...
namespace forte {
  ...
  class CIEC_LDATE final : public CIEC_ANY_DATE {
    ...
    public:
      ...
      constexpr CIEC_LDATE() = default;

      constexpr CIEC_LDATE(const CIEC_LDATE &paValue) : CIEC_ANY_DATE(), mData(paValue.mData) {
        mData = static_cast<TValueType>(static_cast<CIEC_LDATE::TValueType>(paValue));
      }
      ...
      constexpr explicit CIEC_LDATE(const TValueType paValue) : mData(paValue) {
      }
  ...
  constexpr inline CIEC_LDATE operator""_LDATE(unsigned long long int paValue) {
