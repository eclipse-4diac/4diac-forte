 ...
 *   Martin Melik Merkumians - make TForteUInt64 constructor explicit, removed
 *                  built-in type operator=, removed operator++
...
namespace forte {
  ...
  class CIEC_DATE_AND_TIME final : public CIEC_ANY_DATE {
    ...
    public:
      ...
      constexpr CIEC_DATE_AND_TIME() = default;

      constexpr CIEC_DATE_AND_TIME(const CIEC_DATE_AND_TIME &paValue) : CIEC_ANY_DATE(), mData(paValue.mData) {
        mData = static_cast<TValueType>(static_cast<CIEC_DATE_AND_TIME::TValueType>(paValue));
      }

      constexpr explicit CIEC_DATE_AND_TIME(const TValueType paValue) : mData(paValue) {
      }
       ...
       *   This function is necessary for communication with a proper engineering system.
       *   \param paTargetBuf Pointer to the provided buffer
       */
       ...
       *   \param paTargetBuf Pointer to the provided buffer
       */
      void toGMTString(std::string &paTargetBuf) const;
  ...
  constexpr inline CIEC_DATE_AND_TIME operator""_DATE_AND_TIME(unsigned long long int paValue) {
...
namespace std {
  template<>
  struct numeric_limits<forte::CIEC_DATE_AND_TIME>
      : public forte::templates::numeric_limits<forte::CIEC_DATE_AND_TIME> {
